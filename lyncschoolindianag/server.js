...
var appChat = express();
var server = require('http').Server(appChat)
var io = require('socket.io')();
var uuid = require('node-uuid');
var Q = require('q');
var _ = require("underscore")
var dbFunctions = require('./src/server/dbStore/dbFunctions');
...
appChat.get('/ping', function(req, res) {
	res.setHeader('Access-Control-Allow-Origin', config.parentDomain);
	res.send("OK");
});

appChat.get(config.admin_url, function(req, res) {
	res.sendFile(__dirname + '/views/admin.html');
});

appChat.use(express.static(__dirname + '/public'));
